# daily-astronomy-photo — Troubleshooting

### The widget doesn't appear
- Make sure Übersicht is running, then use its menu-bar icon → **Refresh all**.
- Confirm `daily-astronomy-photo.widget` is in
  `~/Library/Application Support/Übersicht/widgets/`.
- Re-run `./install.sh`.

### Images or assets don't load
- Keep the `daily-astronomy-photo.widget` **folder intact**. Übersicht serves bundled assets
